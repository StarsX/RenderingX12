...
static float Halton(const uint32_t i, const uint32_t b)
{
	auto f = 1.0f;
	auto r = 0.0f;
...
static DirectX::XMFLOAT2 Halton(const uint32_t i)
{
	const auto haltonX = Halton(i, 2);
	const auto haltonY = Halton(i, 3);

	return DirectX::XMFLOAT2(haltonX, haltonY);
}

static const DirectX::XMFLOAT2& IncrementalHalton()
{
	static auto haltonBase = DirectX::XMUINT2(0, 0);
	static auto halton = DirectX::XMFLOAT2(0.0f, 0.0f);
